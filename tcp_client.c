...
#else
    static cy_rslt_t connect_to_wifi_ap(char* ssid, char* password);
...
void tcp_secure_client_task(void *arg)
{
    ...
    #else
        /* Connect to Wi-Fi AP */
        if(connect_to_wifi_ap(/*data->ssid*/WIFI_SSID, WIFI_PASSWORD) != CY_RSLT_SUCCESS )
        {
	...
	uint32_t bytes_sent;
	result = cy_socket_send(client_handle, data->jsonPayload, strlen(data->jsonPayload),
								CY_SOCKET_FLAGS_NONE, &bytes_sent);
	if(result != CY_RSLT_SUCCESS)
	{
		printf("Failed to send data. Error code: %"PRIu32"\n", result);
	}
	...
	xTaskNotifyGive(data->hibernate_task_handle);
	vTaskDelete( NULL );
 }
 ...
 *
 *******************************************************************************/
cy_rslt_t connect_to_wifi_ap(char* ssid, char* password)
{
    cy_rslt_t result;
    ...
    memcpy(wifi_conn_param.ap_credentials.SSID, ssid, strlen(ssid));
