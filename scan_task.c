...
void scan_task(void *arg)
{
	...
	{
		result = cy_wcm_start_scan(scan_callback, data, NULL);
	}
	else
	{
		result = cy_wcm_start_scan(scan_callback, data, &scan_filter);
	}
	...
	xTaskNotifyGive(data->tcp_client_task_handle);
	vTaskDelete( NULL );
}
...
static void scan_callback(cy_wcm_scan_result_t *result_ptr, void *user_data, cy_wcm_scan_status_t status)
{
	GLOBAL_DATA* data = (GLOBAL_DATA*)user_data;
    if ((strlen((const char *)result_ptr->SSID) != 0) && (status == CY_WCM_SCAN_INCOMPLETE))
    {
        num_scan_result++;
        print_scan_result(result_ptr);

        if (result_ptr->signal_strength > data->rssi)
        {
        	data->rssi = result_ptr->signal_strength;
        	strncpy(data->ssid, result_ptr->SSID, sizeof(data->ssid)-1);
        }

        if (strlen(data->jsonPayload) == 0)
        	strcat(data->jsonPayload, "[ ");
        else
        	strcat(data->jsonPayload, ", ");

        char tmp[100];
        sprintf(tmp, "{ \"ssid\":\"%s\", \"rssi\": %d }", result_ptr->SSID, result_ptr->signal_strength);
        strcat(data->jsonPayload, tmp);
    }
